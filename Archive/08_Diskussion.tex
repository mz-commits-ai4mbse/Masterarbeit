...
entsteht, der anschließend in ein Zielmodell überführt werden kann.
Diese Beobachtung entspricht der in der Literatur beschriebenen Herausforderung,
...
Engineering-Prozesse einzubetten \cite{Topcu2025,Souza.2026}.
...
grundsätzlich aus der Verarbeitung auszuschließen, sondern Kontext und Funktion
unterschiedlicher Informationsquellen explizit zu unterscheiden
\cite{Bressan.2000}.
...
Der Demonstrationslauf zeigt zusätzlich, dass zwischen
...
kein Inter-Persona-Konflikt besteht. Consensus darf daher nicht grundsätzlich mit hoher Sicherheit oder
fachlicher Richtigkeit gleichgesetzt werden \cite{Sunkaraneni2026}.
Diese Unterscheidung war ebenfalls Gegenstand der formativen Findings und
bleibt für die Gestaltung zukünftiger Review-Unterstützung relevant.
...
Demonstrationslauf zeigt mit der Target-Model-Formulation des
...
Autorisierung getrennte Verantwortung. Der in der Live-Demonstration
erfolgreich durch SYSIDE verarbeitete Output zeigt, dass das
...
Proof-of-Concept-Ausschnitt begrenzt. Der erfolgreiche Demonstrationslauf weist die
...
Die Live-Demonstration und die anschließende Analyse der persistierten
Zustände zeigen, dass der kontrollierte Engineering-Prozess nicht allein
durch das resultierende SysML-v2-Modell beschrieben werden kann. Der in
Abschnitt~\ref{sec:results_traceability} rekonstruierte Informationspfad
zeigt vielmehr, dass auch die während der Demonstration entstandenen
maschinellen Zwischenzustände, menschlichen Entscheidungen und
Modellzustände bis zu ihrer jeweiligen Informationsgrundlage
...
unterscheidbar. Im Demonstrationslauf ist dies beispielsweise an den
...
in \texttt{IEM-000001} vorlag und welche menschlich autorisierte Benennung
und Beschreibung anschließend in \texttt{IEM-000002} übernommen wurde. Traceability
...
Fehlerzustands und der Rohantwort erhalten, obwohl ein späterer
Verarbeitungsversuch erfolgreich war. Der erfolgreiche Zustand ersetzt den
...
neuen zusammengeführten Authority-Artefakt verschmolzen werden. Mehrere autorisierte
...
realer Engineering-Integration ab. Die verwendeten Quellen waren reale
Engineering-Informationen, die gezielt reduziert und aufbereitet wurden.
...
autorisiert werden können \cite{Topcu2025}.
...
\cite{Topcu2025}. Die zentrale Gestaltungsfrage
...
Die Ergebnisse der Live-Demonstration zeigen, dass die vollständige
...
Die abschließende Live-Demonstration fand in Anwesenheit externer Vertreter
der vorgesehenen Zielgruppe statt, darunter der externe Betreuer dieser
Arbeit. Die Bedienung des Prototyps sowie die im System persistierten
fachlichen und modellbezogenen Entscheidungen erfolgten durch den Autor.
Die externe Begleitung erweitert damit den Anwendungskontext der Erprobung,
stellt jedoch keine unabhängige fachliche Validierung oder Nutzerstudie dar.

\paragraph{Datenbasis und empirische Breite}

Die für die formative Evaluation, die Multi-Source-Verifikation und den
abschließenden Demonstrationslauf verwendeten Testdaten basieren auf realen
Engineering-Inhalten von PreciPoint.
...
Anhang~\ref{app:vv_test_data} beziehungsweise für den Demonstrationslauf zusätzlich
...
Fallstudie untersucht. Der erfolgreiche Demonstrationslauf und der
...
\cite{Topcu2025}. Das entwickelte Architekturkonzept
...
Der Demonstrationslauf benötigte für die LLM-gestützten
...
auch die während des Laufs aufgetretenen fehlgeschlagenen Verarbeitungsversuche
und ihre nachfolgenden erneuten Versuche
...
größer gefassten Verarbeitungsschritten auf Quellenebene eine starke Zunahme
...
Kosten war jedoch nicht Gegenstand der Arbeit. Die gemessenen Werte sind daher als
deskriptiver Referenzwert für den untersuchten Demonstrationslauf und nicht als
